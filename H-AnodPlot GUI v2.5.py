...
class AnodPlot:
    ...
    def plotar(self):
        ...
        axes[0].set_xlabel(self.xlabel_pot)
        axes[0].set_ylabel(self.ylabel_pot)
        axes[0].set_title(self.titulo_pot)
        axes[0].legend(facecolor="white", edgecolor="#cccccc", labelcolor="#111111")
        if self.mostrar_grid:
            axes[0].grid(color="#dddddd", linestyle="--", linewidth=0.6)

        axes[1].set_xlabel(self.xlabel_den)
        axes[1].set_ylabel(self.ylabel_den)
        axes[1].set_title(self.titulo_den)
        axes[1].legend(facecolor="white", edgecolor="#cccccc", labelcolor="#111111")
        if self.mostrar_grid:
            axes[1].grid(color="#dddddd", linestyle="--", linewidth=0.6)
    ...
    def gerar_imagens(self, pasta=None, temporario=False):
        ...
        for caminho, attr_y, ylabel_key, titulo_key, xlabel_key in [
            ...
            ax.set_xlabel(getattr(self, xlabel_key))
            ax.set_ylabel(getattr(self, ylabel_key))
            ax.set_title(getattr(self, titulo_key))
    ...
    def gerar_pdf(self, pasta):
        ...
        elementos.append(Image(pot, width=400, height=250))
        elementos.append(Spacer(1, 12))
        elementos.append(Image(den, width=400, height=250))
        doc.build(elementos)
        return caminho_pdf
...
class App:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("H-AnodPlot v2.5")
        try:
    ...
    def _build_pipeline(self, parent):
        ...
        for num, titulo, descricao, cmd in steps:
            ...
            if num == "3":
                ...
                areas_outer.pack(fill="x", padx=8, pady=8)
        ...
        tk.Label(cor_row, text="clicar", font=("Segoe UI", 8),
    ...
    def _build_workspace(self, parent):
        ...
        label(col, "ÁREA DE TRABALHO", font=("Segoe UI", 8, "bold"),
              fg=TEXT_DIM).pack(anchor="w", pady=(0, 10))

        # arquivos 
        self.card_arquivos = card(col)
        self.card_arquivos.pack(fill="x", pady=(0, 6))

        tk.Label(self.card_arquivos, text="Arquivos selecionados",
                 font=FONT_HEAD, fg=TEXT, bg=CARD).pack(anchor="w", padx=14, pady=(10, 4))

        self.lista_frame = tk.Frame(self.card_arquivos, bg=CARD)
        self.lista_frame.pack(fill="x", padx=14, pady=(0, 10))

        tk.Label(self.lista_frame, text="Nenhum arquivo carregado.",
                 font=FONT_SMALL, fg=TEXT_DIM, bg=CARD).pack(anchor="w")

        separator(col).pack(fill="x", pady=4)

        # áreas
        tk.Label(col, text="Áreas dos eletrodos (cm²)",
                 font=FONT_HEAD, fg=TEXT, bg=BG).pack(anchor="w", pady=(4, 4))

        areas_outer = tk.Frame(col, bg=BG)
        areas_outer.pack(fill="x")

        self.scroll_canvas = tk.Canvas(areas_outer, bg=SURFACE,
                                       bd=0, highlightthickness=0, height=120)
        self.scroll_canvas.pack(side="left", fill="both", expand=True)
        ...
        separator(col).pack(fill="x", pady=6)

        # informações da amostra
        label(col, "INFORMAÇÕES DA AMOSTRA", font=("Segoe UI", 8, "bold"),
              fg=TEXT_DIM).pack(anchor="w", pady=(0, 6))

        info_card = card(col)
        info_card.pack(fill="x", pady=(0, 4))

        self.info_var = tk.StringVar(value="Selecione uma amostra no painel abaixo.")
        tk.Label(info_card, textvariable=self.info_var,
                 font=FONT_SMALL, fg=TEXT_DIM, bg=CARD,
                 justify="left", wraplength=420).pack(anchor="w", padx=14, pady=10)

        separator(col).pack(fill="x", pady=6)

        # painel de estilo de linha
        label(col, "ESTILO DA AMOSTRA SELECIONADA", font=("Segoe UI", 8, "bold"),
              fg=TEXT_DIM).pack(anchor="w", pady=(0, 6))
        ...
        tk.Label(sel_row, text="Amostra:", font=FONT_SMALL,
                 fg=TEXT_DIM, bg=CARD, width=12, anchor="w").pack(side="left")
        ...
        self.amostra_menu.config(bg=BG, fg=TEXT, activebackground=ACCENT2,
                                 activeforeground="white", font=FONT_SMALL,
                                 relief="flat", highlightthickness=0, bd=0, width=24)
        self.amostra_menu["menu"].config(bg=BG, fg=TEXT, activebackground=ACCENT2,
                                         activeforeground="white", font=FONT_SMALL)
        self.amostra_menu.pack(side="left", padx=6)

        # tipo de linha
        tipo_row = tk.Frame(estilo_card, bg=CARD)
        tipo_row.pack(fill="x", padx=12, pady=4)

        tk.Label(tipo_row, text="Tipo de linha:", font=FONT_SMALL,
                 fg=TEXT_DIM, bg=CARD, width=12, anchor="w").pack(side="left")
        ...
        estilo_menu.config(bg=BG, fg=TEXT, activebackground=ACCENT2,
                           activeforeground="white", font=FONT_SMALL,
                           relief="flat", highlightthickness=0, bd=0, width=14)
        estilo_menu["menu"].config(bg=BG, fg=TEXT, activebackground=ACCENT2,
                                   activeforeground="white", font=FONT_SMALL)
        estilo_menu.pack(side="left", padx=6)

        # cor da linha
        cor_row = tk.Frame(estilo_card, bg=CARD)
        cor_row.pack(fill="x", padx=12, pady=4)

        tk.Label(cor_row, text="Cor da linha:", font=FONT_SMALL,
                 fg=TEXT_DIM, bg=CARD, width=12, anchor="w").pack(side="left")

        self.cor_preview = tk.Label(cor_row, bg=CORES_TAB10[0], width=4,
                                    relief="flat", cursor="hand2")
        self.cor_preview.pack(side="left", padx=6, ipady=8)
        self.cor_preview.bind("<Button-1>", lambda e: self._escolher_cor())

        tk.Label(cor_row, text="← clique para escolher", font=FONT_SMALL,
                 fg=TEXT_DIM, bg=CARD).pack(side="left", padx=2)

        # espessura
        lw_row = tk.Frame(estilo_card, bg=CARD)
        lw_row.pack(fill="x", padx=12, pady=4)

        tk.Label(lw_row, text="Espessura:", font=FONT_SMALL,
                 fg=TEXT_DIM, bg=CARD, width=12, anchor="w").pack(side="left")

        self.lw_var = tk.DoubleVar(value=1.8)
        lw_slider = tk.Scale(lw_row, from_=0.5, to=5.0, resolution=0.5,
                             orient="horizontal", variable=self.lw_var,
                             bg=CARD, fg=TEXT, troughcolor=BG,
                             highlightthickness=0, activebackground=ACCENT,
                             length=160, command=self._on_lw_change)
        lw_slider.pack(side="left", padx=6)

        self.lw_label = tk.Label(lw_row, text="1.8 pt", font=FONT_SMALL,
                                  fg=TEXT_DIM, bg=CARD, width=5)
        self.lw_label.pack(side="left")

        tk.Frame(estilo_card, bg=CARD, height=8).pack()  # espaçamento inferior
        ...
        label(col, "GRAFICOS", font=("Segoe UI", 8, "bold"),
              fg=TEXT_DIM).pack(anchor="w", pady=(0, 6))
        ...
        self.status_var = tk.StringVar(value="Pronto. Selecione arquivos .dat para começar.")
        status_bar = tk.Frame(self.root, bg=SURFACE, pady=4)
        status_bar.pack(fill="x", side="bottom")
        tk.Label(status_bar, textvariable=self.status_var,
                 font=FONT_SMALL, fg=TEXT_DIM, bg=SURFACE).pack(side="left", padx=16)
    ...
    def _atualizar_menu_amostras(self):
        ...
        for nome in nomes:
            menu.add_command(label=nome,
                             command=lambda n=nome: self._on_amostra_select(n))
    ...
    def _on_amostra_select(self, nome):
        idx = self._idx_amostra(nome)
        if idx is not None:
            self._selecionar_amostra_idx(idx)
    ...
    def _idx_amostra_ativa(self):
        ...
        return self._idx_amostra(self.amostra_var.get())
    ...
    def _atualizar_lista(self):
        ...
        self._atualizar_lista_selecao()
        return
        for arq in self.arquivos_sel:
            row = tk.Frame(self.lista_frame, bg=CARD)
            row.pack(fill="x", pady=2)
            tk.Label(row, text="  📄", font=FONT_BODY, fg=ACCENT, bg=CARD).pack(side="left")
            tk.Label(row, text=os.path.basename(arq), font=FONT_MONO,
                     fg=TEXT, bg=CARD).pack(side="left", padx=4)
